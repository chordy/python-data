# -*- coding: utf-8 -*-
"""
Created on Mon Oct  2 15:09:49 2017

@author: chord
"""

# -*- coding: utf-8 -*-
"""
Created on Sun Oct  1 15:35:19 2017
hmm fit of a trace
input:
    csv data
    estimated mu
    estimated transtion matrix
output:
    decoded lifetime & parameter
    fitting score
    
@author: chord
"""
from matplotlib import pyplot as plt
import numpy as np
from hmmlearn import hmm
import csv
from lifetime_cal import life_time
from lifetime_cal import decay_line as dl
from scipy.optimize import curve_fit  

def exp_func(x, k):  
    return np.exp(-k * x)
def exp2_fun(x,k1,k2,a):
    return a*np.exp(-k1*x)+(1-a)*np.exp(-k2*x)
#from scipy import signal
a=[]
with open('testdata.csv','r') as f:
    reader=csv.reader(f)
    for row in reader:
        a.append(float(row[0]))
high_force=max(a)
low_force=min(a)
#convolution
#win=signal.hann(100)
#b=signal.convolve(a,win,mode='same')/sum(win)
# initial value
data = np.reshape(np.array(a), [-1,1]);               #data for analysis
stat_num=4                                                                # number of states
bias=0.9                                                          # self transiton index
interval=(high_force-low_force)/(stat_num-1)
guess_m=[]
for ii in range(stat_num):
    guess_m.append(low_force+interval*ii)
guess_mu=np.reshape(np.array(guess_m),[-1,1])
init_tij=bias*np.eye(stat_num,stat_num)
startprob=[1.0/stat_num]*int(stat_num)
for ii in range(stat_num):
    for jj in range(stat_num):
        if init_tij[ii][jj]==0:
            init_tij[ii][jj]=(1-bias)/(stat_num-1)       #finish setting init tij
#fitting
model = hmm.GaussianHMM(n_components=int(stat_num), covariance_type="full",n_iter=100, init_params="cs")
#model.startprob_ = startprob
#model.transmat_ = init_tij
model.transmat_=np.array([[  9.97268388e-001,   2.73153479e-003,   7.70829451e-008,
          4.21558242e-139],
       [  5.81225079e-003,   3.69583453e-001,   6.22174630e-001,
          2.42966668e-003],
       [  1.88345462e-007,   2.99392503e-001,   6.90989946e-001,
          9.61736298e-003],
       [  1.35289240e-004,   8.24471625e-003,   2.01404156e-002,
          9.71479579e-001]])
model.means_ = np.array([[7],[8.18],[8.44],[8.65]])
model.fit(data)
hidden_states = model.predict(data)
plt.figure()
plt.plot(data[10000:15000])
hidden_means=[]
for i in range(len(hidden_states)):
    hidden_means.append(model.means_[hidden_states[i]])
plt.plot(hidden_means[10000:15000])
plt.show()
decoded=model.decode(data) #state array
mu=[]
sigma=[]
for i in range(model.n_components):
    mu.append(model.means_[i])
    sigma.append(model.covars_[i])
temp_tij=model.transmat_
logL=model.score(data)
print(temp_tij)
[s,t]=life_time(list(decoded[1]),stat_num)
print(mu,t)
#plot decay_line
dec=[[],[],[],[]]
tim=[[],[],[],[]]
for i in range(stat_num):
    plt.figure()
#bins = np.arange(min(s[0]), max(s[0]), 5); #浮点数版本的range
    tim[i],dec[i]=dl(s[i])
#    popt, pcov = curve_fit(exp_func, tim[i], dec[i])
#    y1=[exp_func(iii,popt[0]) for iii in tim[i]]
#    popt2,pcov2=curve_fit(exp2_fun, tim[i],dec[i])
#    y2=[exp_func(iii,popt[0],popt[1],popt[2]) for iii in tim[i]]
#    plt.figure()
#    plt.plot(tim[i],dec[i])
#    plt.plot(tim[i],y1)
#    plt.plot(tim[i],y2)
#    plt.legend('data','exp1','exp2')
#    print(popt)
#    print(pcov)
