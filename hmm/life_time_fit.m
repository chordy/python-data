...
for i = 1:n
%     exclude=acc_time{i}<5;
%     options1.Exclude=exclude;
%     options2.Exclude=exclude;
    if length(acc_time{i})>5 % need at least 5 observed lifetimes to try to fit it.
        [fit1{i}, gof1{i}] = fit(acc_time{i}', acc{i}', single, options1); %gof is "goodness of fit"
        [fit2{i}, gof2{i}] = fit(acc_time{i}', acc{i}', double, options2);
        %call the paramters by doing fit1(i).k1 etc
        
%% plotting the distributions (useful but not needed if not testing/making
        %figures)
            figure;
            plot(acc_time{i}, acc{i}, '.b');
            hold on
            plot(acc_time{i}, fit1{i}(acc_time{i}), 'r');
            plot(acc_time{i}, fit2{i}(acc_time{i}), 'g');
...
%             figure;
%             plot(t{i}(1:end-1), log(1-cdfn{i}(1:end-1)), '.b');
%             set(gca, 'FontSize', 16, 'FontWeight', 'bold')
%             xlabel('Lifetime (s)  ')
